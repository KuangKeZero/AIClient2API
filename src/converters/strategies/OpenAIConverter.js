...
export class OpenAIConverter extends BaseConverter {
    ...
    _emitOpenAIResponsesToolStart(stateKey, toolState, events) {
        if (toolState.added) {
            return;
        }
        events.push({
            ...
            sequence_number: nextSequenceNumber(stateKey),
            type: "response.output_item.added"
        });
        toolState.added = true;
    }

    _finalizeOpenAIResponsesToolCalls(stateKey, state, events) {
        for (const toolState of state.toolCalls.values()) {
            if (!toolState.added) {
                this._emitOpenAIResponsesToolStart(stateKey, toolState, events);
            }
            if (toolState.done) {
                continue;
            }
    ...
    toOpenAIResponsesStreamChunk(openaiChunk, model, requestId = null) {
        ...
        if (choice.finish_reason) {
            ...
            this._finalizeOpenAIResponsesToolCalls(stateKey, state, events);
